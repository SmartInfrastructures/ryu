...
class OVSBridge(object):
    ...
    #Rate in kb
    def add_rate_limit(self, name, rate, in_out = None):
        burst_percent = 0.1
        rate_kb = int(rate)
        ...
        #Ingress policy rate
        ###
        if in_out == 'ingress_rate' or not in_out:
            print "Update port %s with INgress rate limit = %s kbps" % (name, rate_kb)
        ...
        #Egress Policy rate
        ###
        elif in_out == 'egress_rate' or not in_out:
            print "Update port %s with Egress rate limit = %s kbps" % (name, rate_kb)
            queue_config = []
            config = {}
